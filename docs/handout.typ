...
#context {
  ...
  document.date.display(", [hour]:[minute]")
  linebreak()
  [74. DKK, Dresden]
}
...
- Kontext von Webmapping und MapLibre verstehen
- MapLibre Style Specification verstehen und anwenden
- Praktische Anwendung von MapLibre in einer intaktiven Karte
...
_MapLibre_ ist ein open source fork von mapbox-gl-js. Die JavaScript Bibliothek verwendet die _WebGL_ Technologie (unter Verwendung der GPU) um Karten im Browser darzustellen.
...
Kacheln sind ein gängiges Format für die Bereitstellung von Geodaten im Web. Dabei wird die quadratische Karte pro Zoomstufe in jeweils 4 kleinere Kacheln unterteilt (_Quadtree_).

Vectortiles sind ein Datenformat für das Speichern von Geodaten. Diese Daten bestehen aus Geometrien (Punkten, Linien, Polygonen) und deren Attributen (nicht-räumliche, beschreibende Daten). Im Gegensatz zu Rastertiles (Web Map Service) beinhalten Vectortiles tatsächlich *nur die geometrischen Informationen* und *nicht das Aussehen*. Das Aussehen wird erst im Browser durch die Anwendung von Stilen (_styles_) definiert und gerendert. Das bedeutet, dass die Darstellung erst direkt beim Kartenaufruf erfolgt. Dadurch kann die Karte viel leichter an die individuellen Bedarfe der Kartennutzer*innen bzw. den jeweiligen Nutzungskontex angepasst werden.
...
#grid(
  ...
  [
    ...
    - dynamisch und anpassbar (in real-time)
      - Beispiel: rotierende Labels
      - _light_ und _dark mode_
      - Accessibility: Schriftgröße, Kontrast, Farben (basierend auf Nutzerpräferenzen)
      - Sprache (Internationalisierung)
      - Inhalte (filter als Teil der Style Definition)
    - kontinuierlicher Zoom (niemals verpixelt)
    - geringere Speichergröße der Tiles im Vergleich zu Rastertiles (abhängig von Anzahl der Stile und Vereinfachung auf unterschiedlichenZoomlevels)
  ],
  [
...
- Glyphs: Schriftarten für die Darstellung von Texten auf der Karte
- Sprites: Grafische Symbole für die Darstellung von Punkten oder Linien auf der Karte

=== Sources (Datenquellen)

Sind als Objekt, dargestellt durch einen eindeutigen Schlüssel gekennzeichnet, und enthalten:
- die Art der Datenquelle (z. B. Vectortiles, Rastertiles, GeoJSON, etc.)
- die URL oder den Pfad zu der Datenquelle

=== Layer (Kartenebenen)

Sind ein Array, das die verschiedenen Ebenen der Karte definiert. Jede Ebene enthält Informationen über die Art der Darstellung, die Datenquelle, die für diese Ebene verwendet wird, sowie die Stileigenschaften für die Darstellung.
...
Es gibt unterschiedliche einfache Datentypen in JavaScript, wie z. B. Strings (Zeichenketten), Zahlen, Booleans (wahr/falsch).
...
Die einfachen Datentypen können in komplexere Strukturen organisiert werden, wie z. B. in ein _array_ (Liste) oder ein _object_ (Schlüssel-Wert-Paare).
...
- MapLibre Style Spezifikationen: https://maplibre.org/maplibre-style-spec/
